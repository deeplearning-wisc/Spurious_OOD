...
parser.add_argument('--BN', default =False, type=bool,
                    help='if load BN model')
parser.add_argument('--ntom', default =True, type=bool,
                    help='if load k + 1 model')
parser.add_argument('--need-training', default = False, type=bool,
                    help='if training with ID only is needed')
...
def main():
    ...
    if args.model_arch == "resnet101":
        if not args.BN:
            ...
            if args.ntom: 
                clsfier = clssimp(2048, num_classes + 1)
            else:
    ...
    if args.need_training: 
        ...
        for epoch in range(args.start_epoch, args.epochs):     
            # if args.BN:                               
            #     train(args, model, clsfier, train_loader, criterion, optimizer, epoch, directory, log, distributed = False)
            # else:
            train(args, model, clsfier, train_loader, criterion, optimizer, epoch, directory, log, distributed = False)
    ...
    for test_epoch in test_epochs:
        ...
        if args.ntom:
            print("processing ID")
            in_score = get_ntom_score(args, model, clsfier, val_loader, test_epoch, log)
            print("processing OOD")
            out_score = get_ntom_score(args, model, clsfier, testloaderOut, test_epoch, log)
            save_dir =  f"./ntom_results/{args.name}"
            if not os.path.exists(save_dir):
                os.makedirs(save_dir)
            with open(os.path.join(save_dir, f'ntom_score_at_epoch_{test_epoch}.npy'), 'wb') as f:
                np.save(f, in_score)
                np.save(f, out_score)
        else:
            print("processing ID")
            id_max_energy, id_sum_energy = get_energy(args, model, clsfier, val_loader, test_epoch, log)
            print("processing OOD")
            ood_max_energy, ood_sum_energy = get_energy(args, model, clsfier, testloaderOut, test_epoch, log)
            save_dir =  f"./energy_results/{args.name}"
            if not os.path.exists(save_dir):
                os.makedirs(save_dir)
            with open(os.path.join(save_dir, f'energy_score_at_epoch_{test_epoch}.npy'), 'wb') as f:
                np.save(f, id_max_energy)
                np.save(f, id_sum_energy)
                np.save(f,  ood_max_energy)
...
def train(args, model, clsfier, train_loader, criterion, optimizer, epoch, save_dir, log, distributed = False):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if distributed:  
            NUM_DEVICE = 4
            output = nn.parallel.data_parallel(model, input, device_ids=list(range(NUM_DEVICE)))
            output = nn.parallel.data_parallel(clsfier, output, device_ids=list(range(NUM_DEVICE)))  
        else:    
            output = model(input)
...
def get_energy(args, model, clsfier, val_loader, epoch, log):
    ...
    return max_energy, sum_energy

def get_ntom_score(args, model, clsfier, val_loader, epoch, log):
    scores_debug = AverageMeter()
    model.eval()
    clsfier.eval()
    init = True
    log.debug("######## Start collecting ntom score ########")
    with torch.no_grad():
        for i, (images, labels) in enumerate(val_loader):
            images = images.cuda()
            # labels = labels.cuda().float()
            outputs = model(images)
            outputs = clsfier(outputs)
            score = -1.0 * (torch.sigmoid(outputs)[:,-1]).float().detach().cpu().numpy()
            scores_debug.update(score.mean(), len(labels))  #DEBUG
            if init:
                scores = score
                init = False
            else:
            ...
            if i % args.print_freq == 0:
                log.debug('Epoch: [{0}] Batch#[{1}/{2}]\t'
                    'K+1 scores {scores.val:.4f} ({scores.avg:.4f})'.format(
                        epoch, i, len(val_loader), scores=scores_debug))

...
def compute_traditional_ood(args, epochs):
    print('Natural OOD')
    print('nat_in vs. nat_out')

    known = np.loadtxt(f'{args.base_dir}/{args.in_dataset}/{args.method}/{args.name}/{args.exp_name}/{epochs}_nat/in_scores.txt')
    ...
    for out_dataset in out_datasets:
        novel = np.loadtxt(f'{args.base_dir}/{args.in_dataset}/{args.method}/{args.name}/{args.exp_name}/{epochs}_nat/{out_dataset}/out_scores.txt')

        total += novel.shape[0]
        auroc, aupr, fpr = anom_utils.get_and_print_results(known, novel, f"{out_dataset}", args.method)
        results = cal_metric(known, novel, args.method)
    ...
    print_results(avg_results, args.in_dataset, "All", args.name, args.method)
...
def compute_in(args, epochs):

    known_nat = np.loadtxt(f'{args.base_dir}/{args.in_dataset}/{args.method}/{args.name}/{args.exp_name}/{epochs}_nat/in_scores.txt')
    known_nat_sorted = np.sort(known_nat)
    num_k = known_nat.shape[0]

    threshold = known_nat_sorted[round(0.05 * num_k)]

    known_nat_label = np.loadtxt(f'{args.base_dir}/{args.in_dataset}/{args.method}/{args.name}/{args.exp_name}/{epochs}_nat/in_labels.txt')
...
if __name__ == '__main__':
    ...
    for epochs in args.epochs.split():
        all_fprs[epochs] = compute_traditional_ood(args, epochs)
        compute_in(args, epochs)
